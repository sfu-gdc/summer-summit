...
function resolveFrom(): string {
	if (process.env['COMMITLINT_FROM']) return process.env['COMMITLINT_FROM'];

	const candidates = [process.env['COMMITLINT_BASE'], 'origin/HEAD', 'main'];
	const base = candidates.find((ref): ref is string => !!ref && refExists(ref));
	...
	if (!root) throw new Error('no root commit found');
	return root;
}
...
if (!binRel) throw new Error('commitlint bin not found in its package.json');
