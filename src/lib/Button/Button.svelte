...
<svelte:element
	...
	class={[
		...
		variant === 'spray' &&
			visuals &&
			'disabled:[color:var(--button-disabled-content)] disabled:[background:var(--button-disabled-surface)]',
		className,
	]}
>
	<span
		...
		class={[
			'button-layout inline-flex items-center gap-2',
			visuals ? 'invisible' : 'semantic visible opacity-0',
		]}
	>
		{@render children?.()}
		{@render icon?.()}
